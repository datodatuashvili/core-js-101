...
function timeSpanToString(startDate, endDate) {
  const mlsecDiffer = endDate.getTime() - startDate.getTime();
  const date = new Date(0);
  date.setMilliseconds(mlsecDiffer);
  const formatDate = date.toISOString().slice(11, -1);
  return formatDate;
}
...
function angleBetweenClockHands(date) {
  ...
  const radius = Math.min(angle, 360 - angle) * (Math.PI / 180);
  return radius;
}
