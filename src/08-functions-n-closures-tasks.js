 ...
 *   getPolynom()      => null
 */
function getPolynom(...test) {
  const array = test;
  return function polyFunc(test1) {
    return array.reduce((b, a, index) => (test1 ** (array.length - index - 1)) * a + b, 0);
  };
}
...
function logger(func, logFunc) {
  return function wrapp(...object) {
    const test = JSON.stringify(object);
    const node = test.slice(1, test.length - 1);
    const skelet = `${func.name}(${node})`;
    logFunc(`${skelet} starts`);
    const answer = func(...object);
    logFunc(`${skelet} ends`);
    return answer;
  };
}
